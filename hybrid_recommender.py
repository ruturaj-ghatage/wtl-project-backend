# Data manipulation and model building for the hybrid recommender
import pandas as pd
...
class HybridRecommender:
    def __init__(self, content_based_weight=0.5, collaborative_weight=0.5, k=5):
        ...
        genre_labels = sorted(
            set([genre for genres in self.df['genres_list'] for genre in genres]))
        genre_vectors = []
        for genres in self.df['genres_list']:
            genre_vector = [
                1 if genre in genres else 0 for genre in genre_labels]
    ...
    def recommend(self, movie_name):
        ...
        content_scores = list(enumerate(linear_kernel(
        ...
        for i in indices:
            if i in content_indices:
                scores[i] = scores.get(
                    i, 0) + self.content_based_weight * content_scores[content_indices.index(i)][1]
            if i in cf_indices:
                scores[i] = scores.get(
                    i, 0) + self.collaborative_weight * (1 - distances[0][list(cf_indices).index(i)])
        ...
        for i in scores:
            recommended_movies.append(
                {'id': self.df.iloc[i[0]]['id'], 'title': self.df.iloc[i[0]]['title'], 'score': i[1]})
...
if __name__ == '__main__':
    ...
    if recommendations is not None:
        ...
        print('No recommendations found for movie:', movie_name)
